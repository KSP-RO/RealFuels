...
namespace RealFuels.Tanks
{
	public partial class ModuleFuelTanks : IAnalyticTemperatureModifier, IAnalyticPreview
	{
        ...
        private IEnumerator WaitAndRecalculateMass()
        {
            yield return null;
            yield return null;
            yield return null;
        ...
        partial void OnStartRF(StartState state)
		{
            ...
            Fields[nameof(debug3Display)].guiActive = RFSettings.Instance.debugBoilOff && this.supportsBoiloff;
        ...
        private void CalculateAnalyticInsulationFactor(double insulationFactor)
        {
            ...
            {
                // First (normalize AnalyticLerpRateInternal) * (total skin-internal transfer rate)
                ...
                //part.analyticInternalInsulationFactor =
                //    part.partInfo.partPrefab.analyticInternalInsulationFactor
                //    * (part.heatConductivity / part.partInfo.partPrefab.heatConductivity)
                //    * RFSettings.Instance.analyticInsulationMultiplier;
            }
            else
                // freeze internal analytic temperature adjustments until we have a flight integrator
		...
		private IEnumerator CalculateTankBoiloff(double deltaTime, bool analyticalMode = false)
		{
			...
			if (deltaTime > 0)
			{
                ...
                if (analyticalMode)
                {
                    debug3Display = Utilities.FormatFlux(cooling) + " / " + part.analyticInternalInsulationFactor.ToString("F16");
                    Fields[nameof(debug3Display)].guiName = "Analytic Cooling";
                }
                else
                {
                    debug3Display = part.analyticInternalInsulationFactor.ToString("F16");
                    Fields[nameof(debug3Display)].guiName = "analyticInsulation";
                }
				...
				for (int i = tankList.Count - 1; i >= 0; --i)
				{
					...
					if (tank.vsp > 0.0 && tank.totalArea > 0)
					{
						...
						if (double.IsNaN(lossAmount))
                            DebugLog(tank.name + " lossAmount is NaN!");
						else
						{
        ...
        */
		...
		public void CalculateTankArea()
		{
            ...
            {
                DebugLog("Bad " + part.name + ".totalTankArea; sanitizing totalTankArea");
                totalTankArea = tempTotal;
                if (double.IsNaN(part.temperature))
                {
                    DebugLog("Also, part.temperature was FUCKING  NaN - OMG WHY WHY WHY YOU FUCKING HERETIC");
                    if (partPrevTemperature > -1)
                        part.temperature = partPrevTemperature;
                    else if (supportsBoiloff)
                        part.temperature = lowestTankTemperature;
                }
            }
		}
