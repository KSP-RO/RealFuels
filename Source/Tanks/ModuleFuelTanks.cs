					...
					if (tank.amount > 0d)
					{
						if (tank.vsp > 0.0)
						{
                            ...
                            if (MFSSettings.ferociousBoilOff)
                                deltaTemp = ((part.thermalMass / (part.thermalMass - part.resourceThermalMass)) * part.temperature) - tank.temperature;
                            else
                                deltaTemp = part.temperature - tank.temperature;
                            ...
                            if (deltaTemp > 0)
                            {
                                ...
                                // part.thermalMass is lower than overall thermal mass so should heat up faster than the tank contents. 
                                // This simulates a higher part temperature by increasing the temperature delta by the ratio of of total part tmass to tank tmass.
                                // Note that this is incorrect. This should be applied to the part temperature before calculating temp delta but this would increase boiloff rate tremendously
                                // Which I doubt would go over well.
                                //if (MFSSettings.ferociousBoilOff)
