...
namespace RealFuels
{
    public class ModuleEnginesRF : ModuleEnginesSolver
    {
        ...
        public override string GetInfo()
        {
            ...
            output.Append($"{GetThrustInfo()}");
            output.AppendLine($"<b>Engine Isp: </b>{atmosphereCurve.Evaluate(1):0.###} (ASL) - {atmosphereCurve.Evaluate(0):0.###} (Vac.)");
            output.AppendLine($"{GetUllageIgnition()}");
            if (ratedBurnTime > 0d)
            {
                output.Append("<b>Rated Burn Time: </b>");
                if (ratedContinuousBurnTime > 0d)
                    output.AppendLine($"{ratedContinuousBurnTime:F0}/{ratedBurnTime:F0}s");
                else
                    output.AppendLine($"{ratedBurnTime:F0}s");
            }
            output.AppendLine($"<b><color=#99ff00ff>Propellants:</color></b>");
            ...
            foreach (Propellant p in propellants)
            {
                ...
                output.AppendLine($"- <b>{KSPUtil.PrintModuleName(p.name)}</b>: {sUse} maximum.");
                output.Append($"{p.GetFlowModeDescription()}");
            }
            output.AppendLine($"<b>Variance: </b>{localVaryIsp:P2} Isp, {localVaryFlow:P1} flow, {localVaryMixture:P2} MR (stddev).");
            output.AppendLine($"<b>Residuals: min </b>{localResidualsThresholdBase:P1} of propellant.");

            if (!allowShutdown) output.Append("\n<b><color=orange>Engine cannot be shut down!</color></b>");
            if (!allowRestart) output.Append("\n<b><color=orange>If shutdown, engine cannot restart.</color></b>");
